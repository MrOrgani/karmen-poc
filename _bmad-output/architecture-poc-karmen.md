...
> **Note d'évolution.** L'arborescence initiale ci-dessous (`components/`, `red-flags/`) est conservée à titre documentaire ; l'arbo _livrée_ est décrite en §2bis. Les évolutions actées en cours d'exécution (RuleEngine, arbo feature-based, DTO étendu, 10 indicateurs, score-dans-décision) sont signalées par des encarts **« Évolution »** dans les sections concernées.
...
type AugmentedDossier = {
  ...
  // ─── Enrichissement POC ─────────────────────────────
  financialIndicators: {
    // extrait OCR liasse fiscale (simulé)
    revenue: number; // CA N (€)
  ...
  factoringIndicators?: {
    // présent ssi financing_request.type === 'factoring' (simulés)
...
**Gating Hybrid Option 2** : si `requires` n'est pas satisfait, la règle ne déclenche pas un red flag mais remonte `status: 'unknown'` + `unavailableReason` lisible (ex. _"Liasse N-1 manquante"_) — affiché sur la tuile en gris explicite.
...
> **Taxonomie à jour** (post-rename `dossier→case` / `relance→follow-up`, commit 9ffe046, et post-audit [annexe 3](../README-annexe-3-audit-instrumentation.md)). `cockpit.section.expanded` a été retiré (vestigial après le refacto grille plate).
...
// Fire-and-forget : sendBeacon si dispo, sinon fetch keepalive. Ne throw jamais.
...
### Côté backend (`tracking.middleware.ts` + `decisions.controller.ts`)

Middleware NestJS qui émet `http.request` avec sa `durationMs` native. `decision.made` est poussé **server-side** par le `DecisionsController` (verdict + justification + statut), pas par le client. Endpoints `POST /events` (ingest) + `GET /events` (dump JSON, in-memory pour le POC).
...
| **`RuleEngine` comme source unique** _(évolution)_       | Cohérence garantie entre redFlags, tuiles, popovers, bullets — un seul endroit où ajouter une règle                        |
| **Gating Hybrid Option 2** _(évolution)_                 | Préfère un diagnostic _honnête mais incomplet_ à un diagnostic _complet mais extrapolé_                                    |
| **Arbo `features/`** _(évolution)_                       | Regroupement par feature (composants + hooks + types) : un dossier = une feature complète, plus simple à lire et supprimer |
| **React Query** _(évolution post-bloc 2 review)_         | Élimine race conditions, ajoute cache aller/retour liste↔cockpit, retry intelligent (skip 404). +10 KB gzip assumés        |
