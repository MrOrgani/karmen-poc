...
describe('RuleEngine.redFlags — règles prêt', () => {
  ...
  it('DEBT_TO_EBITDA_HIGH se déclenche pour dette > 5× EBITDA', () => {
    ...
    const flag = flags(fin, healthyBank).find(
      (f) => f.code === 'DEBT_TO_EBITDA_HIGH',
    );
  ...
  it('EBITDA_MARGIN_LOW se déclenche pour marge < 5%', () => {
    ...
    expect(
      flags(fin, healthyBank).find((f) => f.code === 'EBITDA_MARGIN_LOW'),
    ).toBeDefined();
  });
  ...
  it('NEGATIVE_NET_INCOME se déclenche pour résultat net négatif', () => {
    ...
    expect(
      flags(fin, healthyBank).find((f) => f.code === 'NEGATIVE_NET_INCOME'),
    ).toBeDefined();
  });
  ...
  it('REVENUE_DECLINING se déclenche pour CA < 90% N-1', () => {
    ...
    expect(
      flags(fin, healthyBank).find((f) => f.code === 'REVENUE_DECLINING'),
    ).toBeDefined();
  });

  it('OVERDRAFT_DAYS_HIGH se déclenche au-delà de 30 jours', () => {
    const flag = flags(healthyFin, {
      ...healthyBank,
      overdraftDaysLast12m: 45,
    }).find((f) => f.code === 'OVERDRAFT_DAYS_HIGH');
    expect(flag).toBeDefined();
    expect(flag?.severity).toBe('high');
  });
  ...
  it('REJECTED_PAYMENTS se déclenche pour ≥1 rejet', () => {
    expect(
      flags(healthyFin, { ...healthyBank, rejectedPaymentsCount: 2 }).find(
        (f) => f.code === 'REJECTED_PAYMENTS',
      ),
    ).toBeDefined();
  });
  ...
  it('LOW_CASH_POSITION se déclenche quand cash < sorties moy', () => {
    ...
    expect(
      flags(fin, bank).find((f) => f.code === 'LOW_CASH_POSITION'),
    ).toBeDefined();
  });

  it('DSO_LONG se déclenche au-delà de 60j (severity medium par défaut = loan)', () => {
    const flag = flags({ ...healthyFin, dso: 75 }, healthyBank).find(
      (f) => f.code === 'DSO_LONG',
    );
    expect(flag).toBeDefined();
    expect(flag?.severity).toBe('medium');
  ...
  it('EBITDA_NEGATIVE_OR_ZERO se déclenche pour EBITDA ≤ 0 (et supprime les ratios dette/EBITDA)', () => {
    const out = flags(
      { ...healthyFin, ebitda: 0, totalDebt: 50000 },
      healthyBank,
    );
...
describe('RuleEngine — règles factoring', () => {
  ...
  it("aucune règle factoring n'est émise pour un dossier prêt, même si factoring fourni", () => {
    const flags = engine.redFlags({
      ...
      financingType: 'loan',
      factoring: {
        topClientConcentrationPct: 90,
        agedReceivablesPct: 90,
        dilutionRatePct: 90,
      },
    });
    expect(
      flags.find((f) => f.code === 'CONCENTRATION_TOP_CLIENT'),
    ).toBeUndefined();
    expect(
      flags.find((f) => f.code === 'AGED_RECEIVABLES_HIGH'),
    ).toBeUndefined();
