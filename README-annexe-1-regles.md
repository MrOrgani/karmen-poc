# Annexe 1 — Le `RuleEngine` : pourquoi *ces* 10 règles, et ce qu'on ajouterait
...
**(b) 3 règles `category: 'factoring'`** activées *uniquement* si `type === 'factoring'` :
...
Sur un dossier prêt, les 3 tuiles factoring sont **absentes du diagnostic** (pas affichées "unknown" — purement filtrées). Sur *Fleurs de Saison* (seul dossier factoring), elles apparaissent dans une 3e section "Qualité créances (affacturage)" sous les sections financière et bancaire.

Couvert par 7 tests unitaires (`backend/src/red-flags/red-flags.spec.ts`) : pondération DSO loan/factoring, déclenchement des 3 règles, absence sur les dossiers prêt, comportement `unknown` si `factoringIndicators` absent.

---

## Pourquoi pas *tous* les indicateurs factoring possibles ?
...
1. **Différence additive, pas substitutive** : Grégoire l'a validé en kickoff (« faible différenciation, mêmes étapes + quelques indicateurs *en plus* pour le factoring »). Les règles factoring **complètent** le socle commun, elles ne le remplacent pas.
