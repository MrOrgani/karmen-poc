...
export function MetricTile({
  ...
  const effectiveStatus: MetricStatus | undefined = unavailableReason
    ? "unknown"
    : status;
  const style = effectiveStatus
    ? STATUS_STYLES[effectiveStatus]
    : {
        wrapper: "bg-karmen-pale-blue/40 border-karmen-border-blue/40",
        label: "text-karmen-mute",
      };
  ...
  return (
    <div className={cn("rounded-lg border p-3", style.wrapper)}>
          ...
          <Popover>
            ...
            >
              <div>
                ...
                <p className="font-medium text-karmen-marine tabular-nums mt-0.5">
                  {threshold.rule}
                </p>
              </div>
              <div>
      ...
      {unavailableReason ? (
        <>
          <div className="text-lg font-semibold text-karmen-mute mt-1 leading-tight">
            —
          </div>
          <div className="text-xs text-karmen-mute mt-0.5 italic">
            {unavailableReason}
          </div>
        </>
      ) : (
        <>
          <div className="text-lg font-semibold text-karmen-ink mt-1 leading-tight tabular-nums">
            {value}
          </div>
          {hint && (
            <div className="text-xs text-karmen-mute mt-0.5 tabular-nums">
              {hint}
            </div>
          )}
        </>
