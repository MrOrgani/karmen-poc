...
import type { AugmentedCase, CompletenessResult } from "@/shared/types";
...
type Props = {
  ...
  documents: AugmentedCase["documents"];
};

const TYPE_LABEL: Record<AugmentedCase["documents"][number]["type"], string> = {
  liasse_fiscale: "Liasse fiscale",
  releve_bancaire: "Relevé bancaire",
};

function metaLine(doc: AugmentedCase["documents"][number]): string {
  ...
  if (doc.metadata.months_covered !== undefined)
    parts.push(`${doc.metadata.months_covered} mois`);
  return parts.join(" · ");
}
...
function DocIcon({
  ...
  type: AugmentedCase["documents"][number]["type"];
}) {
    ...
    >
      <div className="space-y-5">
        <section aria-labelledby="docs-provided">
          <h3
            id="docs-provided"
            className="text-xs uppercase tracking-widest text-karmen-mute font-semibold mb-2"
          >
          ...
          {documents.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-karmen-ink truncate min-w-0">
                        {doc.name}
                      </div>
                      <div className="text-xs text-karmen-mute truncate min-w-0">
                        {metaLine(doc)}
                      </div>
                    </div>
                  </div>
            ...
            <p className="text-sm text-karmen-mute italic">
              No document provided for this case.
            </p>
          )}
        </section>
        ...
        {missing.length > 0 && (
          <section aria-labelledby="docs-missing">
            <h3
              id="docs-missing"
              className="text-xs uppercase tracking-widest text-amber-800 font-semibold mb-2"
            >
            ...
            <ul className="space-y-2 rounded-lg border border-amber-300 bg-amber-50 p-3">
              {missing.map((item, idx) => (
                <li
                  key={`${item.type}-${idx}`}
                  className="flex items-start gap-2 text-sm text-karmen-ink"
                >
                  <AlertCircle
                    aria-hidden
        ...
        <div className="flex justify-end">
          <Button
            ...
            className={
              isComplete
                ? "border-karmen-border-blue"
                : "bg-karmen-blue hover:bg-karmen-blue-dark"
            }
