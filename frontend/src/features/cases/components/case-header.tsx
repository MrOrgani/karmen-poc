...
import type { AugmentedCase, RiskBucket } from '@/shared/types';
import { cn } from '@/shared/lib/utils';
import { formatCurrency, formatMonths, formatPercent } from '@/shared/lib/format';

type Props = { case: AugmentedCase };

const TYPE_LABEL: Record<AugmentedCase['financing_request']['type'], string> = {
  loan: 'Prêt',
    ...
    <header className="space-y-4">
      ...
      <div className="rounded-2xl bg-karmen-pale-blue border border-karmen-border-blue/60 p-6 md:p-8">
        <div className="flex items-start gap-4">
          ...
          <div
            className={cn(
              ...
              SCORE_CHIP_TONE[caseData.score.risk_bucket],
            )}
            aria-label={`Score ${caseData.score.global_score} sur 100, ${BUCKET_LABEL[caseData.score.risk_bucket]}`}
          >
            <span aria-hidden className={cn('h-2 w-2 rounded-full', SCORE_DOT[caseData.score.risk_bucket])} />
            <span className="tabular-nums font-semibold">{caseData.score.global_score}</span>
            <span className="text-xs opacity-70">/100</span>
            <span className="hidden md:inline text-xs opacity-70">· {BUCKET_LABEL[caseData.score.risk_bucket]}</span>
          </div>
        </div>
        ...
        >
          <span aria-hidden className={cn('h-2 w-2 rounded-full', SCORE_DOT[caseData.score.risk_bucket])} />
          <span className="tabular-nums font-semibold">{caseData.score.global_score}</span>
          <span className="text-xs opacity-70">/100 · {BUCKET_LABEL[caseData.score.risk_bucket]}</span>
        </div>
