      ...
      <div className="rounded-2xl bg-karmen-pale-blue border border-karmen-border-blue/60 p-6 md:p-8">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-karmen-mute mt-1">
              {company.legalCategory} · {company.businessType} · SIREN{" "}
          ...
          >
            <span
              ...
              className={cn(
                ...
                SCORE_DOT[caseData.score.risk_bucket],
              )}
            />
            <span className="tabular-nums font-semibold">
              {caseData.score.global_score}
            </span>
            <span className="text-xs opacity-70">/100</span>
            <span className="hidden md:inline text-xs opacity-70">
              · {BUCKET_LABEL[caseData.score.risk_bucket]}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <span className="text-xs opacity-70">
            /100 · {BUCKET_LABEL[caseData.score.risk_bucket]}
          </span>
        </div>
        ...
        <dl className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 pt-6 border-t border-karmen-border-blue/60">
          ...
          <Stat
            label="Durée"
            value={formatMonths(req.durationInMonth)}
            numeric
          />
...
function Stat({
  ...
  return (
    <div>
      <dt className="text-xs uppercase tracking-widest text-karmen-mute font-medium">
        {label}
      </dt>
      <dd
        className={[
          "mt-1",
          tone === "primary"
            ? "text-karmen-blue font-semibold text-lg"
            : "text-karmen-marine font-medium",
